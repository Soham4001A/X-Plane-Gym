...
def explained_variance(y_pred: np.ndarray, y_true: np.ndarray) -> np.ndarray:
    ...
    :return: explained variance of y_true
    """
    assert y_true.ndim == 1 and y_pred.ndim == 1
    var_y = np.var(y_true) # <--- THIS IS WHERE THE ERROR IS
    if var_y == 0:
        # Avoid division by zero
        # Explained variance is undefined in this case
        return np.array(np.nan)
    return np.array(1 - np.var(y_true - y_pred) / var_y)
